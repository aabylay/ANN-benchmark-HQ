...
def plot_throughput_vs_recall_by_probes(averages, plots_dir, att_idx):
    ...
    for qt in averages['query_type'].unique():
        ...
        for j, algo in enumerate(sub_qt['algorithm'].unique()):
            for i, clusters in enumerate(sub_qt['clusters'].unique()):
                sub = sub_qt[(sub_qt['algorithm'] == algo) & (sub_qt['clusters'] == clusters)]
                grouped = sub.groupby('probes').agg({
                    'recall': 'mean',
                    'throughput': 'mean'
                }).reset_index()
                grouped = grouped.sort_values(by='probes')
                label = f'{algo} clusters={clusters}'
                if algo == 'pgvector_ivf': algo_idx = 1
                elif algo == 'milvus-ivfflat': algo_idx = 0
                elif algo == 'faiss-ivf': algo_idx = 2
        ...
        plt.title(f'Throughput vs Recall by probes (att_idx={att_idx}, @k=10) - {qt}')
        plt.yscale('log')
        plt.yticks([10**i for i in range(1, int(np.ceil(np.log10(max(sub_qt['throughput'])))) + 1)])
        plt.legend()
        plt.grid(True, which="both", ls="--")
        plt.savefig(os.path.join(plots_dir, f'throughput_vs_recall_by_probes_{qt}_attidx_{att_idx}.png'))
        plt.savefig(os.path.join(plots_dir, f'throughput_vs_recall_by_probes_{qt}_attidx_{att_idx}.pdf'))
        plt.close()

def plot_recall_vs_probes_by_selectivity(averages, plots_dir, att_idx):
    averages = averages[averages['k'].isin([10])]
    averages = averages[averages['clusters'].isin([10])]
    for qt in averages['query_type'].unique():
        plt.figure(figsize=(10, 6))
        sub_qt = averages[averages['query_type'] == qt]
        ...
        for idx, sel in enumerate(unique_sel[:len(markers_sel)]):
            ...
            for j, algo in enumerate(sub_sel['algorithm'].unique()):
                ...
                plt.plot(grouped['probes'], grouped['recall'], marker=markers_sel[idx], color=colors_sel[algo_idx][idx], label=label)
        plt.xlabel('probes')
        plt.ylabel('Recall')
        plt.title(f'Recall vs probes by selectivity (att_idx={att_idx}, @clusters=10, @k=10) - {qt}')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(plots_dir, f'recall_vs_probes_by_selectivity_{qt}_attidx_{att_idx}.png'))
        plt.close()

def plot_throughput_vs_recall_by_selectivity_fixed_k_clusters(averages, plots_dir, att_idx):
    
    print(averages["clusters"].unique())
    print(averages["k"].unique())
    averages = averages[averages['clusters'].isin([5, 15])]
    for qt in averages['query_type'].unique():
        sub_qt = averages[averages['query_type'] == qt]
        for k_val in [1, 10, 100]:
            for clusters_val in [750, 1600]:
                plt.figure(figsize=(10, 6))
                sub = sub_qt[(sub_qt['k'] == k_val) & (sub_qt['clusters'] == clusters_val)]
                ...
                for idx, sel in enumerate(unique_sel[:len(markers_sel)]):
                    ...
                    for j, algo in enumerate(sub_sel['algorithm'].unique()):
                        if algo == 'milvus-ivfflat': color = '#FF0000'
                        elif algo == 'pgvector_ivf': color = '#0000FF'
                        else: color = '#00FF00'
                        sub_algo = sub_sel[sub_sel['algorithm'] == algo]
                        grouped = sub_algo.groupby('probes').agg({
                            'recall': 'mean',
                            'throughput': 'mean'
                        }).reset_index()
                ...
                plt.xlabel('Recall')
                plt.ylabel('Throughput (queries per second)')
                plt.title(f'Throughput vs Recall by selectivity, @k={k_val}, @clusters={clusters_val}, @att_idx={att_idx} - {qt}')
                plt.yscale('log')
                plt.yticks([10**i for i in range(1, int(np.ceil(np.log10(max(sub['throughput'])))) + 1)])
                plt.legend()
                plt.grid(True, which="both", ls="--")
                plt.savefig(os.path.join(plots_dir, f'throughput_vs_recall_by_selectivity_k_{k_val}_clusters_{clusters_val}_{qt}_attidx_{att_idx}.png'))
                plt.close()
            ...
            for k_val in [10]:
                for clusters_val in [750, 1600]:
                    ...
                    for att_idx in [0]:
                        ...
                        sub_qt = averages[(averages['query_type'] == qt) & (averages['algorithm'] == algo) & (averages['k'] == k_val) & (averages['clusters'] == clusters_val)]
                        unique_sel = sorted(sub_qt['filter_selectivity'].unique())
                        for idx, sel in enumerate(unique_sel[:len(markers_sel)]):
                            sub_sel = sub_qt[sub_qt['filter_selectivity'] == sel]
                            grouped = sub_sel.groupby('probes').agg({
                                'recall': 'mean',
                                'throughput': 'mean'
                            }).reset_index()
                            grouped = grouped.sort_values(by='recall')
                    ...
                    plt.xlabel('Recall')
                    plt.ylabel('Throughput (queries per second)')
                    plt.title(f'Throughput vs Recall by selectivity, @k={k_val}, @clusters={clusters_val} - {algo} - {qt} - {dataset_size}')
                    plt.yscale('log')
                    max_throughput = max(averages_dict[0][(averages_dict[0]['query_type'] == qt) & (averages_dict[0]['algorithm'] == algo) & (averages_dict[0]['k'] == k_val) & (averages_dict[0]['clusters'] == clusters_val)]['throughput'])
                    plt.yticks([10**i for i in range(1, int(np.ceil(np.log10(max_throughput))) + 1)])
                    plt.legend()
                    plt.grid(True, which="both", ls="--")
                    plt.savefig(os.path.join(plots_dir, f'throughput_vs_recall_by_selectivity_{algo}_k_{k_val}_clusters_{clusters_val}_{qt}_{dataset_size}.png'))
                    plt.close()
...
def main(dataset_size):
    ...
    for att_idx in [0]:
        ...
        plot_throughput_vs_recall_by_probes(averages, plots_dir, att_idx)
        plot_recall_vs_probes_by_selectivity(averages, plots_dir, att_idx)
        plot_throughput_vs_recall_by_selectivity_fixed_k_clusters(averages, plots_dir, att_idx)
