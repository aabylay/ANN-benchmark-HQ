"""
VLDB paper plots: large datasets only.
- combined_throughput_vs_recall_by_selectivity (2 rows x 2 cols) - from make_plots_results_ALL.py
...
- hnsw_throughput_vs_recall_by_selectivity_attidx (1 row x 2 cols) - from make_plots_attidx_comparison_ALL.py, pgvector only
"""
import pandas as pd
...
# 2 rows x 2 cols: [movies-HNSW, movies-IVF], [reviews-HNSW, reviews-IVF]
# =============================================================================
...
# pgvector only, large dataset, 2 rows x 2 cols: rows=[movies, reviews], cols=[HNSW, IVF]
# =============================================================================
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Plot 1: combined_throughput_vs_recall_by_selectivity - 2 rows x 2 cols (large only)
    # Rows: movies, reviews | Cols: HNSW, IVF
    # -------------------------------------------------------------------------
    fig1, axes1 = plt.subplots(2, 2, figsize=(16, 9))  # 2 rows, 2 cols
    all_metadata_hnsw = []
    all_metadata_ivf = []
    ...
    for row_idx, query_type in enumerate(['movies', 'reviews']):
        if averages_hnsw is not None:
            handles, labels, metadata = plot_throughput_vs_recall_by_selectivity_hnsw(
                axes1[row_idx, 0], averages_hnsw, DATASET_SIZE, query_type, return_handles_labels=True)
            if row_idx == 0:
                all_metadata_hnsw.extend(metadata)
        if averages_ivf is not None:
            handles, labels, metadata = plot_throughput_vs_recall_by_selectivity_ivf(
                axes1[row_idx, 1], averages_ivf, DATASET_SIZE, query_type, return_handles_labels=True)
            if row_idx == 0:
                all_metadata_ivf.extend(metadata)

    # Unified legend (use HNSW metadata; same system names apply to both HNSW and IVF)
    legend_handles, legend_labels = create_unified_legend_throughput_recall(
        all_metadata_hnsw, [], algo_names_hnsw)
    fig1.legend(legend_handles, legend_labels, loc='center left', bbox_to_anchor=(0.85, 0.5), ncol=1)
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(os.path.join(plots_dir, 'vldb_combined_throughput_vs_recall_by_selectivity.pdf'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, 'vldb_combined_throughput_vs_recall_by_selectivity.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plot 1 saved: {plots_dir}/vldb_combined_throughput_vs_recall_by_selectivity.pdf/png")
    ...
    # 2 rows x 2 cols: rows=[HNSW, IVF], cols=[movies, reviews] | Legend on the right
    # -------------------------------------------------------------------------
    ...
    if has_hnsw or has_ivf:
        fig_attidx, axes_attidx = plt.subplots(2, 2, figsize=(15, 9))
        all_metadata_hnsw_att, all_metadata_ivf_att = [], []
        # Transposed: rows=[HNSW, IVF], cols=[movies, reviews]
        for col_idx, query_type in enumerate(['movies', 'reviews']):
            if has_hnsw:
                handles, labels, metadata = plot_throughput_vs_recall_attidx_hnsw(
                    axes_attidx[0, col_idx], hnsw_attidx_0, hnsw_attidx_1,
                    DATASET_SIZE, query_type, algorithm='pgvector', return_handles_labels=True
                )
                all_metadata_hnsw_att.extend(metadata)
                axes_attidx[0, col_idx].set_title(f'{query_type.capitalize()}, HNSW')
            if has_ivf:
                handles, labels, metadata = plot_throughput_vs_recall_attidx_ivf(
                    axes_attidx[1, col_idx], ivf_attidx_0, ivf_attidx_1,
                    DATASET_SIZE, query_type, algorithm='pgvector_ivf', return_handles_labels=True
                )
                all_metadata_ivf_att.extend(metadata)
                axes_attidx[1, col_idx].set_title(f'{query_type.capitalize()}, IVF')

        # Unified legend on the right: HNSW and IVFFlat sections with headers
        legend_handles, legend_labels = [], []
        # Invisible handle for section headers (matplotlib needs a handle per label)
        blank_handle = Line2D([0], [0], color='none', marker='none', linestyle='', alpha=0)

        def build_legend_entries(metadata):
            legend_data = {}
            for handle, att_idx, sel_idx, sel_value, qt in metadata:
                key = (att_idx, sel_idx)
                if key not in legend_data:
                    legend_data[key] = {'handle': handle, 'sel_values': {}}
                legend_data[key]['sel_values'][qt] = sel_value
            for att_idx, sel_idx in sorted(legend_data.keys(), key=lambda x: (x[0], x[1])):
                data = legend_data[(att_idx, sel_idx)]
                sel_dict = data['sel_values']
                sel_strs = [f'{qt}: {sel_dict[qt]:.2f}' if sel_dict[qt] >= 0.01 else f'{qt}: {sel_dict[qt]:.3f}'
                            for qt in ['movies', 'reviews'] if qt in sel_dict]
                sel_label = '{' + ', '.join(sel_strs) + '}'
                legend_handles.append(data['handle'])
                legend_labels.append(f'att_idx={att_idx}, $\\sigma_g$={sel_label}')

        if all_metadata_hnsw_att:
            legend_handles.append(blank_handle)
            legend_labels.append("HNSW (row 1):")
            build_legend_entries(all_metadata_hnsw_att)
        if all_metadata_ivf_att:
            legend_handles.append(blank_handle)
            legend_labels.append("\nIVFFlat (row 2):")
            build_legend_entries(all_metadata_ivf_att)

        fig_attidx.legend(legend_handles, legend_labels, loc='center left', bbox_to_anchor=(0.84, 0.5), ncol=1)
        plt.tight_layout(rect=[0, 0, 0.85, 1])
        plt.savefig(os.path.join(plots_dir, 'vldb_hnsw_throughput_vs_recall_by_selectivity_attidx_pgvector.pdf'), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(plots_dir, 'vldb_hnsw_throughput_vs_recall_by_selectivity_attidx_pgvector.png'), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Plot 2b saved: {plots_dir}/vldb_hnsw_throughput_vs_recall_by_selectivity_attidx_pgvector.pdf/png")
    else:
    ...
    # -------------------------------------------------------------------------
    fig3, axes3 = plt.subplots(2, 3, figsize=(16, 9))  # 2 rows, 3 cols
    all_metadata = []
    ...
    if averages_hnsw is not None and averages_ivf is not None:
        ...
        # Add explanation row for M/R abbreviations
        legend_handles.append(Line2D([0], [0], color='none', marker='none', linestyle=''))
        legend_labels.append('M — Movies, R — Reviews')

        fig3.legend(legend_handles, legend_labels, loc='center left', bbox_to_anchor=(0.98, 0.52),
                    ncol=1, handlelength=6, fontsize=22,
                    handler_map={tuple: mpl.legend_handler.HandlerTuple(ndivide=None)})

    plt.tight_layout(rect=[0, 0.06, 1, 1], w_pad=-0.6)
    plt.savefig(os.path.join(plots_dir, 'vldb_combined_hnsw_vs_ivf_comparison.pdf'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, 'vldb_combined_hnsw_vs_ivf_comparison.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plot 3 saved: {plots_dir}/vldb_combined_hnsw_vs_ivf_comparison.pdf/png")
        ...
        if corr_df is not None and len(corr_df) > 0:
            ...
            for idx_type, ls in [('HNSW', '-'), ('IVF', '--')]:
                ...
                for level in ['high', 'average', 'low']:
                    ...
                    if level == 'high':
                        legend_labels.append(f'{idx_type}, Positive GLS correlation')
                    elif level == 'average':
                        legend_labels.append(f'{idx_type}, Neutral GLS correlation')
                    else:
                        legend_labels.append(f'{idx_type}, Negative GLS correlation')

            fig4.legend(legend_handles, legend_labels,
                        handler_map={tuple: HandlerTuple(ndivide=None)},
                        loc='center left', bbox_to_anchor=(0.87, 0.52), ncol=1,
                        handlelength=5, fontsize=22)

            plt.tight_layout(rect=[0, 0, 0.88, 1], w_pad=0.2)
            plt.savefig(os.path.join(plots_dir, 'vldb_qps_recall_by_correlation.pdf'), dpi=300, bbox_inches='tight')
            plt.savefig(os.path.join(plots_dir, 'vldb_qps_recall_by_correlation.png'), dpi=300, bbox_inches='tight')
            plt.close()
            print(f"Plot 4 saved: {plots_dir}/vldb_qps_recall_by_correlation.pdf/png")
        else:
