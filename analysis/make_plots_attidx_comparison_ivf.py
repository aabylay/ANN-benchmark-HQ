...
def plot_throughput_vs_recall_by_selectivity_attidx(ax, averages_0, averages_1, dataset_size, query_type='movies', algorithm='pgvector_ivf', return_handles_labels=False):
    """Plot: Throughput vs recall by selectivity comparing att_idx=0 vs att_idx=1 (k=10)"""
    markers_sel = ['D', 'X', 'o']
    ...
    if len(averages_0) > 0:
        ...
        # Verify filtering: ensure all rows have k=10
        if len(sub_qt_0) > 0:
...
def main():
    ...
    algorithms = [('pgvector_ivf', 'pgvector'), ('milvus-ivfflat', 'Milvus')]
    ...
    for algorithm, algo_display in algorithms:
        ...
        plt.savefig(os.path.join(plots_dir, f'ivf_throughput_vs_recall_by_selectivity_attidx_{algo_display.lower()}.pdf'), 
                   dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(plots_dir, f'ivf_throughput_vs_recall_by_selectivity_attidx_{algo_display.lower()}.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Plot saved: {plots_dir}/ivf_throughput_vs_recall_by_selectivity_attidx_{algo_display.lower()}.png/pdf")
