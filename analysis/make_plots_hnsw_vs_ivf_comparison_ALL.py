...
# System order for combined figure: (a) FAISS, (b) Milvus, (c) pgvector
...
def plot_hnsw_ivf_comparison(ax, hnsw_averages, ivf_averages, dataset_size, query_type='movies', system_name='Milvus', return_handles_labels=False):
    ...
    ax.set_ylabel('QPS')
    ax.set_title(f'{query_type.capitalize()} ({data_sizes[dataset_size][query_type]})')
    ax.set_yscale('log')
...
def main():
    ...
    # =========================================================================
    # Create combined figure with all 3 systems
    # GridSpec with 8 rows: 2 for FAISS, gap, 2 for Milvus, gap, 2 for pgvector
    # =========================================================================
    fig = plt.figure(figsize=(24, 27.75))
    gs = GridSpec(8, 3, figure=fig, 
                  height_ratios=[1, 1, -0.12, 1, 1, -0.12, 1, 1], 
                  hspace=0.63, wspace=0.5, top=0.945, bottom=0.06)
    
    # Create axes array: 6 logical rows (2 per system) x 3 columns
    # Map logical rows to GridSpec rows (skip gap rows 2 and 5)
    axes = np.empty((6, 3), dtype=object)
    gs_row_map = {0: 0, 1: 1, 2: 3, 3: 4, 4: 6, 5: 7}  # Skip rows 2 and 5 (gaps)
    for row in range(6):
        for col in range(3):
    ...
    # Plot each system
    for sys_idx, system_name in enumerate(system_order):
        base_row = sys_idx * 2  # Each system takes 2 logical rows
        ...
        for row_idx, query_type in enumerate(['movies', 'reviews']):
            for col_idx, dataset_size in enumerate(dataset_sizes):
                ...
                # Only collect metadata from first system for legend
                if sys_idx == 0:
                    all_metadata.extend(metadata)
    
    # Add section titles (positioned in the gaps)
    section_labels = ['(a) FAISS', '(b) Milvus', '(c) pgvector']
    # Y positions for each section title (above each 2-row section)
    title_y_positions = [0.96, 0.645, 0.325]
    
    for label, y_pos in zip(section_labels, title_y_positions):
        fig.text(0.5, y_pos, label, ha='center', va='bottom', fontsize=28, fontweight='bold')
    
    # Create multi-color legend using tuple handles
    # Each legend entry will show 3 colored lines (one per system) side by side
    
    # Collect selectivity values from metadata
    legend_data = {}
    ...
    # Markers and linewidths for each selectivity index
    markers_sel = ['.', '+', 'x', 's']
    base_linewidth = 2
    linewidth_multipliers = [0.8, 0.9, 1.4, 2]
    ...
    for index_type, sel_idx in sorted_keys:
        ...
        # Choose color dict based on index type
        colors = system_colors_hnsw if index_type == 'HNSW' else system_colors_ivf
        
        # Create a tuple of 3 Line2D objects (one per system) displayed together
        handles_tuple = tuple(
    ...
    fig.legend(legend_handles, legend_labels, loc='upper center', bbox_to_anchor=(0.5, 0.02), 
               ncol=2, fontsize=20, handlelength=6, 
               handler_map={tuple: mpl.legend_handler.HandlerTuple(ndivide=None)})
    
    plt.savefig(os.path.join(plots_dir, 'combined_hnsw_vs_ivf_comparison.pdf'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(plots_dir, 'combined_hnsw_vs_ivf_comparison.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Combined plot saved: {plots_dir}/hnsw_vs_ivf_comparison_combined.pdf/png")
